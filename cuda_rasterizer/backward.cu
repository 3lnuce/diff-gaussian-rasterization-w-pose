...
__global__ void computeCov2DCUDA_Fast(int P,
	const float3* means,
	const int* radii,
	const float* cov3Ds,
...
template<int C>
__global__ void preprocessCUDA_Fast(
	int P, int D, int M,
	const float3* means,
	const int* radii,
	const float* shs,
...
renderCUDA(
	...
	bool is_init,
	uint32_t* __restrict__ total_ops,
	uint32_t* __restrict__ skip_ops)
{
...
#ifdef PROFILE
	__shared__ int total_ops_;
	__shared__ int skip_ops_;
	total_ops_ = 0;
	skip_ops_ = 0;
#endif
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; j < min(BLOCK_SIZE, toDo); j++) {
			...
			// float diff = -2 * power;	// DM^2
			// // printf ("debug diff %f\n", diff);
			// // diff = diff * 3.f;

			// if (d.x * d.x + d.y * d.y >= 64)
			// {
			// 	if (!(std::abs(diff - 0.5) < 1e-2))
			// 	// if (diff >= 2)
			// 	{
			// 		// printf ("skipping %f\n", diff);
			// 		// continue;
	...
	if (!is_init)
	{
		// printf ("Total ops: %d, skip ops: %d\n", total_ops_, skip_ops_);
		if (tid == 0)
		{
			atomicAdd(total_ops, total_ops_);
			atomicAdd(skip_ops, skip_ops_);
		}
	}
#endif
}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
			...
			for (int ch = 0; ch < C; ch++)
			{
				...
				dL_dalpha += (c - accum_rec[ch]) * dL_dchannel;
				if (!skip_aggre)
					atomicAdd(&(dL_dcolors[global_id * C + ch]), dchannel_dcolor * dL_dchannel * 2);
			...
			if (!skip_aggre)
				atomicAdd(&dL_ddepths[global_id], dchannel_dcolor * dL_dpixel_depth * 2);
			...
			if (!skip_aggre)
			{
				atomicAdd(&dL_dmean2D[global_id].x, dL_dG * dG_ddelx * ddelx_dx * 2);
				atomicAdd(&dL_dmean2D[global_id].y, dL_dG * dG_ddely * ddely_dy * 2);

				atomicAdd(&dL_dconic2D[global_id].x, -0.5f * gdx * d.x * dL_dG * 2);
				atomicAdd(&dL_dconic2D[global_id].y, -0.5f * gdx * d.y * dL_dG * 2);
				atomicAdd(&dL_dconic2D[global_id].w, -0.5f * gdy * d.y * dL_dG * 2);
	...
	const int* is_active)
{
	...
	computeCov2DCUDA_Fast << <(P + 255) / 256, 256 >> > (
		P,
		means3D,
		radii,
		cov3Ds,
